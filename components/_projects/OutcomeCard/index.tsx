import { type OutcomeProps, type OutcomeStatProps } from '@/types/blockTypes'
...
export const OutcomeStat: React.FC<OutcomeStatProps> = ({ number, label, numLabel }) => {
  return (
    <div data-cursor-pointer="text" className="h-40">
      ...
      >{`${number}${numLabel}`}</div>
      <div data-testid="stat-label" className="font-medium text-lg">
        {label}
      </div>
    </div>
  )
  ...
  return (
    ...
    >
      <div className="relative z-50 opacity-100 flex flex-col justify-between h-full grow">
        <div data-testid="result-type" className="font-medium text-darkGrey/70 dark:text-pillGrey">
          {outcomeHash[resultType]}
        </div>
        <div data-testid="emojis" className="font-medium text-2xl min-h-8">
          {emojis}
        </div>
        <div>
